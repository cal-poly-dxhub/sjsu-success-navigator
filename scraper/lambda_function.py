...
A missing or malformed crawl list is fatal on purpose, and both change gates matter.
"""
...
# The rest of the enum is COMPLETE, FAILED and STOPPED, none of which occupies the source.
ACTIVE_INGESTION_STATUSES = ("STARTING", "IN_PROGRESS", "STOPPING")

DEFAULT_URL_LIST_FILE = "urls.csv"

# /opt is where Lambda extracts the layer; the function's own directory keeps a local run working.
...
def seed_list_path(filename=None) -> Path:
    """Or a SeedListError naming everywhere it looked."""
...
def content_fingerprint(markdown, metadata) -> str:
    """Over everything a run would upload for one page, timestamp excluded."""
...
def stored_content_fingerprint(s3, bucket, key):
    """None means treat the page as changed."""
    try:
        head = s3.head_object(Bucket=bucket, Key=key)
    except Exception as exc:  # noqa: BLE001 (an absent object is the normal case)
...
def latest_object_modified(s3, bucket):
    """None when the bucket is unreadable or empty."""
    latest = None
    try:
...
def _metadata_body(metadata: dict) -> bytes:
    """The `.md.metadata.json` name is load-bearing: Bedrock looks for exactly it."""
    attributes = {
...
def expected_kb_keys(urls):
    """What the KB source bucket should hold after a healthy run."""
    keys = set()
    for url in urls:
...
def prune_stale_objects(s3, bucket, expected_keys):
    """Deletes the KB source objects the crawl list no longer calls for."""
    deleted: list[str] = []
    try:
        paginator = s3.get_paginator("list_objects_v2")
        for page in paginator.paginate(Bucket=bucket):
...
def _ingestion_job_summaries(bedrock_agent, kb_id, data_source_id, max_results=20):
    """Newest first, and empty when unreadable."""
    try:
...
def last_ingestion_started_at(summaries):
    """Started-at rather than finished-at, because it can only over-trigger."""
    started = None
    for job in summaries:
...
def should_start_ingestion(changed_this_run, bucket_latest, last_started):
    """The decision and the reason to log. Never raises."""
    if changed_this_run:
    ...
    if bucket_latest is not None and last_started is not None and bucket_latest > last_started:
        # Content no job has covered: almost always a run that deferred behind another job.
...
def start_ingestion(
    ...
    """Only when one is warranted and none is running. Never raises."""
    ...
    try:
        ...
        # A racing job or a throttle: both are the overlap case, and both self-heal next run.
...
def handler(event, context):
    # Unguarded on purpose: a SeedListError fails the invocation before anything is deleted.
    ...
    # Unioned into the prune, so a run can never delete a page it just looked at.
    ...
    for result in results:
        ...
        # Gate 1, upload only what changed. The sidecar rides along: its timestamp always moves.
    ...
    # Before ingestion, so one job indexes the new content and retires the removed.
    ...
    # Gate 2, ingest only when the bucket moved or still holds unindexed content.
    ...
    # One structured line per run. duration_seconds is here for the timeout question.
    summary = {
